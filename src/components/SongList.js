...
const SongList = ({song, loaded}) => {
    ...
    // assign feed of all entries to fullList variable 
    const fullList = song.feed.entry;
    // map the feed of all entries to a new array using the map method  
    const newList = fullList.map((song) => 
        <li className="list-item" key={song["im:name"]["label"]}>
            <a href={song.link[0].attributes.href} target="_blank" rel="noreferrer">
                <img src={song["im:image"][2]["label"]} alt={song["title"]["label"]}></img>
            </a>
    ...
    return(
        ...
        <div className="container-list">
            <div className="logo-apple-music">
            ...
            {/* the new array of all entries is output in an ordered list */}
            <ol>{newList}</ol>
        </div>
        </>
    )
